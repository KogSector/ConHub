...
async function resetDatabase() {
    ...
    const doDrop = args.includes('--drop') || !doClean; // default remains drop for backward compatibility
    ...
    try {
        ...
        if (doClean) {
            // Discover all public tables and truncate with cascade
            ...
            if (tables.length === 0) {
                console.log('ℹ️  No tables found in public schema to clean');
            } else {
                const quoted = tables.map(t => `"${t}"`).join(', ');
                const truncateSql = `TRUNCATE TABLE ${quoted} RESTART IDENTITY CASCADE;`;
                await client.query(truncateSql);
                console.log('✅ All tables truncated with RESTART IDENTITY CASCADE');
                console.log('🎉 Database cleaned without dropping tables');
            }
        }
