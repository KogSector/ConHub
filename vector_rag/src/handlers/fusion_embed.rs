...
pub async fn fusion_embed_handler(
    ...
    // Generate embeddings using fusion service
    match service.generate_embeddings(&texts, "general_text").await {
        Ok(mut embeddings) => {
            if req.normalize {
                VectorOps::batch_normalize(&mut embeddings);
            }
            let dimension = embeddings.get(0).map(|e| e.len()).unwrap_or(0);
        ...
        Err(e) => {
            log::error!("Fusion embedding generation failed: {}", e);
