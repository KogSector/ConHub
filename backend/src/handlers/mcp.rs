...
static mut MCP_SERVER: Option<Arc<ConHubMcpServer>> = None;
static mut MCP_CLIENT_MANAGER: Option<Arc<McpClientManager>> = None;
...
pub async fn initialize_mcp_server(req: web::Json<CreateMcpServerRequest>) -> Result<HttpResponse> {
    unsafe {
        ...
        match server.initialize().await {
            Ok(_) => {
                MCP_SERVER = Some(Arc::new(server));
                MCP_CLIENT_MANAGER = Some(Arc::new(McpClientManager::new()));
